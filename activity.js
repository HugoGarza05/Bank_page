...
(function () {
  ...
  const balanceInput = $("#balanceInput"); // optional numeric input
  ...
  function render() {
    ...
    // account balance display (supports #balanceInput and/or #accountBalance)
    const total = rows.reduce((sum, r) => sum + (Number(r.amount) || 0), 0);
    if (balanceInput) balanceInput.value = total.toFixed(2);
    if (balanceEl) balanceEl.textContent = fmtUSD(total);
  }
